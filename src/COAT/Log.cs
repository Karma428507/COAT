...
public class Log
{
    ...
    public static void LogLevel(Level level, string msg)
    {
        ...
        {
            case Level.Info:
                Logger.Info(level == Level.Debug ? $"<color=#BBBBBB>{msg}</color>" : msg);
                break;
            case Level.Debug:
                Logger.Debug(level == Level.Debug ? $"<color=#BBBBBB>{msg}</color>" : msg);
                break;
            case Level.Warning:
                Logger.Warning(level == Level.Debug ? $"<color=#BBBBBB>{msg}</color>" : msg);
                break;
            default:
                Logger.Error(level == Level.Debug ? $"<color=#BBBBBB>{msg}</color>" : msg);
                break;
        }
